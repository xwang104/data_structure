...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while (singleQueue.size() + mergeQueue.size() > 1) {
        TreeNode* fst = removeSmallest(singleQueue, mergeQueue);
        TreeNode* snd = removeSmallest(singleQueue, mergeQueue);
        TreeNode* newNode = new TreeNode((fst->freq).getFrequency() + (snd->freq).getFrequency());
        newNode -> left = fst;
        newNode -> right = snd;
