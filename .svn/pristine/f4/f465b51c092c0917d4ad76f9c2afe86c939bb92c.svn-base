...
class KDTree
{
  ...
  private:
    ...
    int partition(vector<Point<Dim>> & pts, int left, int right, int pivotIndex, int curDim);
    void select(vector<Point<Dim>> & pts, int left, int right, int curDim, int n);
    void buildTree(int left, int right, int curDim);
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, int left, int right, int curDim) const;
};
