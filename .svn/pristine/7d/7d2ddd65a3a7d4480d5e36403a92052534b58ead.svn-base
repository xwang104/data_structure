...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    Dict<char, int> hashTable1(1);
    Dict<char, int> hashTable2(1);
    for (size_t i = 0; i < word.length(); i++)
        hashTable1[word[i]]++;
    for (size_t i = 0; i < test.length(); i++)
        hashTable2[test[i]]++;

    if (hashTable1.tableSize() != hashTable2.tableSize())
        return false;

    typename Dict<char, int>::iterator it;
    for (it = hashTable1.begin(); it != hashTable1.end(); it++) {
        if (!hashTable2.keyExists(it->first))
            return false;
        if (hashTable2.find(it->first) != it->second)
            return false;
    }
    return true;
