...
void BinaryTree<T>::mirror(Node* subRoot) {
    ...
    // mirror the left tree
    mirror(subRoot->left);

    // mirror the right tree
...
{
    vector<int> prev;
    printPaths(prev, root);
}

template <typename T>
void BinaryTree<T>::printPaths(vector<int> prev, const Node* subRoot) const {
    if (subRoot == NULL)
        return;
    else
    ...
    if (subRoot->left != NULL)
        printPaths(prev, subRoot->left);
    if (subRoot->right != NULL)
        printPaths(prev, subRoot->right);
}
...
template <typename T>
vector<int> BinaryTree<T>::sumDistances(const Node*subRoot) const {
    vector<int> res = {0, 0};
    if (subRoot == NULL)
        return res;
    vector<int> l = sumDistances(subRoot->left);
    vector<int> r = sumDistances(subRoot->right);
    res[0] = l[0] + l[1] + r[0] + r[1];
    res[1] = l[1] + r[1] + 1;
    return res;
}
